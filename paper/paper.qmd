...
@sec-model-details] provides model detail, and [Appendix- @sec-survey]
provides an detailed introduction of surveys, sampling and observational
data in our paper.
...
model to quantify the impact of abortion injunctions on the infant death
...
reflects the policy intervention rather than changes in population
composition.

#### Long-term Effects vs. Reliability

The length of the analysis period further complicates the reliability of
results. While longer periods allow for capturing long-term effects,
...
-   **Time Indicator `after_injunction`**: A binary variable indicating
    the time period: 0 for the period before the injunction implemented.
    1 for the period after the injunction implemented.
...
-   **`state`**: Mother's residence state when the infant born as a
...
model’s implementation are provided in [Appendix- @sec-model-details].

## Model set-up

In our analysis, we employ two models. The first model includes only the
predictor variables and the random effect for state. The second model
...
these controls to focus on the overall intervention impact, avoid
...
two models is to compare the outcomes: one adjusts for confounders to
...
priors based on data scale. This enhances convergence and ensures priors
...
modelsummary::msummary(
  ...
  fmt = 3,coef_omit="state",notes = "Note. All models include a random variable for state. The reference level of age_of_mother is 15-19.The reference level of monthers_single_race is Asian. Source: CDC Wonder")
```
...
coefficient for Black mothers in Model 2 is 6.66, reflecting higher
...
period result in an additional increase of 0.111 deaths in Model 1 and
...
In @fig-ppcheckandposteriorvsprior-1 and @fig-ppcheckandposteriorvsprior-2 we implement a posterior predictive check for the first model and second model. It represent posterior predictive checks for Model 1 and Model 2, illustrating how well each model captures the observed data. The posterior predictive checks for Model 1 (policy-focused) and Model 2 (policy and demographic factors) illustrate model fit. In @fig-ppcheckandposteriorvsprior-1, the observed ($y$) and replicated ($y_{rep}$) distributions align moderately, reflecting general trends but limited precision. @fig-ppcheckandposteriorvsprior-2 shows improved alignment, indicating that adding demographic factors enhances predictive accuracy and better captures the complexities of infant mortality rates.
...
to finding respondents. Respondent-driven sampling (RDS), developed by
...
trust, fostering higher participation rates in populations that may
